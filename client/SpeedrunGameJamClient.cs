...
using SpeedrunGameJam;
using SpeedrunGameJam.Program;
using SpeedrunGameJam.Errors;
using SpeedrunGameJam.Accounts;

namespace SpeedrunGameJam
{
    namespace Accounts
    {
    ...
    namespace Errors
    {
        public enum SpeedrunGameJamErrorKind : uint
        {
        }
    }

    public partial class SpeedrunGameJamClient : TransactionalBaseClient<SpeedrunGameJamErrorKind>
    {
        public SpeedrunGameJamClient(IRpcClient rpcClient, IStreamingRpcClient streamingRpcClient, PublicKey programId) : base(rpcClient, streamingRpcClient, programId)
        {
        }
        ...
        {
            Solana.Unity.Rpc.Models.TransactionInstruction instr = Program.SpeedrunGameJamProgram.InitGameStateForPlayer(accounts, programId);
            return await SignAndSendTransaction(instr, feePayer, signingCallback);
        }

        public async Task<RequestResult<string>> SendUpdateGameStateAsync(UpdateGameStateAccounts accounts, ushort ore, ushort crystal, ushort platinum, PublicKey feePayer, Func<byte[], PublicKey, byte[]> signingCallback, PublicKey programId)
        {
            Solana.Unity.Rpc.Models.TransactionInstruction instr = Program.SpeedrunGameJamProgram.UpdateGameState(accounts, ore, crystal, platinum, programId);
            return await SignAndSendTransaction(instr, feePayer, signingCallback);
        }
        ...
        {
            Solana.Unity.Rpc.Models.TransactionInstruction instr = Program.SpeedrunGameJamProgram.Reset(accounts, programId);
            return await SignAndSendTransaction(instr, feePayer, signingCallback);
        }

        protected override Dictionary<uint, ProgramError<SpeedrunGameJamErrorKind>> BuildErrorsDictionary()
        {
            return new Dictionary<uint, ProgramError<SpeedrunGameJamErrorKind>>{};
        }
    }
    ...
    namespace Program
    {
        ...
        public class UpdateGameStateAccounts
        {
            public PublicKey Signer { get; set; }
        ...
        public static class SpeedrunGameJamProgram
        {
            ...
            public static Solana.Unity.Rpc.Models.TransactionInstruction UpdateGameState(UpdateGameStateAccounts accounts, ushort ore, ushort crystal, ushort platinum, PublicKey programId)
            {
                List<Solana.Unity.Rpc.Models.AccountMeta> keys = new()
                ...
                byte[] _data = new byte[1200];
                int offset = 0;
                _data.WriteU64(17934765600815565664UL, offset);
                offset += 8;
